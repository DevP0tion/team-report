...
분석 범위에 따라 전문가 서브에이전트를 3~10개 동적으로 구성해 병렬 분석을 수행하고, Accept/Defer/Reject 결정 카드와 피드백 루프를 갖춘 단일 파일 HTML 리포트를 생성하는 Claude Code 스킬입니다. 결정 사항은 `localStorage`에 저장되며 `docs/reports/feedback.yaml`에 누적되어 이후 분석 품질을 점진적으로 개선합니다.
